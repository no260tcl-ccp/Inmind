  ...
  <div class="page-container">
    ...
    <div class="page-content">
      ...
      <div class="menu-grid">
        ...
        <div class="menu-item" @click="router.push('/pages-coffeeb/myFormula/myFormula')">
...
//const showDialog = ref(false) // 精準對齊您 Template 裡的 van-dialog 綁定
...
// 建議您在打開這個彈窗的函式（例如 recipeShow = true）的後面呼叫這個 loadLocalRecipes()
...
const openSaveDialog = () => {
  ...
  // 💡 安全防呆：同時把兩個常見的控制變數都設為 true，確保絕對能打開您的 van-dialog 彈窗！
  if (typeof showDialog !== 'undefined') showDialog.value = true
...
const confirmSaveRecipe = () => {
  ...
  // 💡 【核心黑魔法】：由於舊 UI 只有單個溫度、粉水量輸入
  // 我們在後端「自動」幫使用者將其優化拆解為標準手沖的「黃金兩段式（悶蒸+主沖）」
  const totalWater = currentPowder.value * currentRatio.value
  const bloomWater = Math.min(totalWater, currentPowder.value * 2) // 悶蒸水量通常固定為粉量兩倍 (例如 30ml)
  const mainWater = Math.max(0, totalWater - bloomWater)          // 剩下的大隊伍水量分配到第一段
  ...
  const newRecipe: Recipe = {
    ...
    temp: currentTemp.value, // 保留根節點方便舊 UI 渲染渲染 
    grind: true,             // 預設開啟研磨
    gear: 3,                 // 預設經典研磨檔位
    speed: 90,               // 預設標準轉速 90 RPM
    stages: autoGeneratedStages // 注入自動生成的完美雙段數據陣列！
  }
...
const downloadCloudRecipe = (recipe: Recipe) => {
  const cloned: Recipe = {
    ...JSON.parse(JSON.stringify(recipe)), // 深拷貝防止連動
    id: 'local_' + Date.now()
  }
...
const shareRecipeToDevice = (recipe: Recipe) => {
  ...
  // Data 4~8: 基礎參數封包
  recipePkg[7] = recipe.ratio / 2      // 協議規範：粉水比需除以 2 傳輸
  recipePkg[8] = recipe.powder         // 豆粉量 (g)
  recipePkg[9] = recipe.grind ? 1 : 0  // 研磨開關
  recipePkg[10] = recipe.gear          // 研磨檔位
  recipePkg[11] = recipe.speed         // 研磨轉速

  // Data 9: 讀取高階陣列中的實際有效段數
  // 即使是用舊 UI 存下來的，經過上面 confirmSaveRecipe 的洗禮，這裡也會拿到正確的 2 段
  const totalStages = recipe.stages ? recipe.stages.length : 1
  recipePkg[12] = totalStages

  // Data 10~34: 【重頭戲】完美參考 grind.vue，用 for 迴圈動態遍歷發送最高 5 段注水參數
  let offset = 13
  for (let i = 0; i < 5; i++) {
    if (recipe.stages && i < totalStages) {
      const stage = recipe.stages[i]
      recipePkg[offset] = stage.water                                   // 水量 (ml)
      recipePkg[offset + 1] = Number(`3.${stage.velocity}`) * 10        // 流速公式對位
      recipePkg[offset + 2] = stage.temperature                         // 該段獨立溫度
      recipePkg[offset + 3] = stage.type                                // 注水方式
      recipePkg[offset + 4] = stage.time                                // 停駐秒數 / 悶蒸秒數
    } else {
      // 剩下的空白協議空間，精準補上零，阻絕亂碼干擾
    ...
    offset += 5 // 每段佔 5 個 Bytes，指針後推
  }

  // Data 35~54: 將中文配方名稱轉為二進位碼寫入封包尾端
...
onMounted(() => {
  ...
  if (saved) {
    try {
      // 從瀏覽器硬碟取出時，系統會自動載入高階的 stages 陣列物件
