            ...
            <van-swipe-cell v-for="(item, index) in dataList" :key="item.id || index" class="mb-4">
              ...
              <div class="formula-item" @click.stop="itemClick(item)">
                ...
                <div class="formula-item-right">
                  <div class="recipe-temp">{{ item.configJson?.accordionItems?.[0]?.temperature || 92 }}°C</div>
                  <div class="stage-badge">
                    {{ item.configJson?.accordionItems?.length || 2 }}<span class="stage-unit">段</span>
                  </div>
                </div>
              </div>
...
/**
 * 🌟 核心修正：點擊卡片跳轉詳情頁 (拔除 encodeURIComponent 防止雙重編碼)
 */
/**
 * 🌟 點擊卡片跳轉詳情頁 (Web 最佳實踐：快遞傳參)
 */
const itemClick = (item: any) => {
  console.log('--- 準備導航至配方詳情頁 ---', item.name)
  if (!item) return

  let finalConfig = item.configJson || {}
  if (typeof finalConfig === 'string') {
    try { finalConfig = JSON.parse(finalConfig) } catch (e) { finalConfig = {} }
  }

  const params = { configJson: finalConfig, id: item.id, isEdit: false, name: item.name }
  
  // ✨ 終極黑魔法：將巨量資料暫存在本地快遞櫃，不塞進網址裡！
  localStorage.setItem('bincoo_transit_data', JSON.stringify(params))

  // 乾淨俐落的跳轉，完全沒有 query 的編碼負擔！
  router.push({
    path: '/pages-coffeeb/formulaDetail/formulaDetail'
  }).catch(err => {
    alert('🚨 跳轉被攔截：' + err.message)
  })
}

// 🌟 同步更新詳情頁解析邏輯：從本地快遞櫃取資料
const initRecipeData = () => {
  try {
    // ✨ 從本地快遞櫃取出剛剛存入的乾淨資料
    const transitData = localStorage.getItem('bincoo_transit_data')
    if (!transitData) {
      console.warn('沒有接收到配方快遞資料')
      return
    }

    const params = JSON.parse(transitData)
...
const editRecipe = (item: any) => {
  ...
  const params = { configJson: finalConfig, id: item.id, isEdit: true, name: item.name }
  router.push({
    path: '/pages-coffeeb/formula/formula',
    query: { data: JSON.stringify(params) }
  }).catch(err => alert('編輯跳轉失敗：' + err.message))
}
