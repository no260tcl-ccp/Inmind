...
/**
 * 🌟 核心防線：智能相容解析，並優先從資料庫撈取最新鮮的資料！
 */
onMounted(() => {
  try {
    let routeId = route.query.id as string
    let isEditMode = route.query.isEdit === 'true'
    let routeName = ''

    // 1. 🛡️ 雙模相容：如果列表頁傳來的是舊版 `data` 物件，在這裡解開它
    if (route.query.data) {
      const routeParams = JSON.parse(decodeURIComponent(route.query.data as string))
      isEditMode = routeParams.isEdit
      routeId = routeParams.id
      routeName = routeParams.name || ''
    }

    isEdit.value = isEditMode
    id.value = isEditMode ? routeId : null
    name.value = routeName

    // 2. 🚀 終極同步：只要有 ID，就不要相信網址帶來的舊快取，自己去 LocalStorage 挖出最新資料！
    if (routeId) {
      const localStr = localStorage.getItem('bincoo_my_recipes')
      if (localStr) {
        const allRecipes = JSON.parse(localStr)
        const targetRecipe = allRecipes.find((r: any) => String(r.id) === String(routeId))
        ...
        if (targetRecipe && targetRecipe.configJson) {
          ...
          console.log('✨ 成功從資料庫撈出最新修改資料！')
          return // 成功載入最新資料，直接結束初始化，不會掉進下面的創建模式
        }
      }
    }

    // 3. 🆕 如果上面沒有 return，代表是真正的「全新創建」
    isEdit.value = false
    totalWaterVolume.value = Math.round((legumes.value * proportion.value) / 2)
...
/**
 * 🌟 保存配方：精準存入 LocalStorage，並觸發列表自動更新
 */
const addFormula = () => {
  let flag = false
  let totalValue = 0
  accordionItems.value.forEach((item) => {
    if (item.water == 0 || item.temperature == 0 || item.time == 0) {
      flag = true
    }
  ...
  if (totalWaterVolume.value !== totalValue) {
    showToast('水量分配不等于预计总水量')
    return
  }
  ...
  try {
    ...
    // 3. 延遲返回列表頁，完美觸發 myFormula.vue 的監聽器
    setTimeout(() => {
      router.back()
    }, 600)
...
const negativeCorrection = computed(() => {
  return totalWaterVolume.value - currentTotalWater.value
})

const pushItems = () => {
  if (accordionItems.value.length >= 5) {
    showToast('最多支持 5 段注水')
    return
  }
  accordionItems.value.push({
...
const handleUpdateWater = (val: any) => {
  accordionItems.value[val.index].water = val.val
}
const handleUpdateTemperature = (val: any) => {
  accordionItems.value[val.index].temperature = val.val
}
const handleUpdateVelocity = (val: any) => {
  accordionItems.value[val.index].velocity = val.val
}
const handleUpdateTime = (val: any) => {
  accordionItems.value[val.index].time = val.val
}
