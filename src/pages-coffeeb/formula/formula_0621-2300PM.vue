        ...
        <div class="mr-3 ml-3 mt-4">
          <div v-if="accordionItems.length > 0">
            <AccordionItem
              ...
              @updateWaterInjectPercentage="handleUpdateWaterInjectPercentage"
...
onMounted(() => {
  ...
  if (queryData) {
    try {
      ...
      accordionItems.value = (config.accordionItems || []).map((item: any) => {
...
const handleUpdateWater = (val: any) => {
  if (accordionItems.value[val.index]) {
    accordionItems.value[val.index].water = Number(val.newValue || 0)
  }
}

// 💡 實作您提議的專屬接收事件！精準覆寫百分比
const handleUpdateWaterInjectPercentage = (val: any) => {
  if (accordionItems.value[val.index]) {
...
const autoWater = () => {
  ...
  const currentWater = Number(accordionItems.value[activeIndex.value].water || 0)
  const newWater = currentWater + maxWater
  
  accordionItems.value[activeIndex.value].water = newWater
  accordionItems.value[activeIndex.value].percentage = totalWaterVolume.value > 0 ? Math.round((newWater / totalWaterVolume.value) * 100) : 0
  
  nowWater.value += maxWater
}
...
watch([legumes, proportion], ([newLegumes, newProportion]) => {
  ...
  accordionItems.value.forEach((item) => {
    item.percentage = total > 0 ? Math.round((Number(item.water) / total) * 100) : 0
  })
})
