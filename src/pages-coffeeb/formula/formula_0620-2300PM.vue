...
onMounted(() => {
  ...
  if (queryData) {
    try {
      ...
      accordionItems.value = config.accordionItems || []
...
// 💡 完全還原微信小程序寫法：不解構、使用 toFixed(0) 字串型態
const handleUpdateWater = (val: any) => {
  accordionItems.value[val.index].water = val.newValue
  accordionItems.value.forEach((item) => {
    item.percentage = ((item.water / totalWaterVolume.value) * 100).toFixed(0)
	//console.info('讀取 percentage:', item.percentage)
  })
}

// 💡 完全還原微信小程序寫法：子組件會負責算總量並傳過來
...
const autoWater = () => {
  ...
  const maxWater = remainingWater > 240 ? 240 : remainingWater
  accordionItems.value[activeIndex.value].water += maxWater
  accordionItems.value[activeIndex.value].percentage = (
    (accordionItems.value[activeIndex.value].water / totalWaterVolume.value) *
    100
  ).toFixed(0)
