  ...
  if (queryData) {
    try {
      ...
      percentagesList.value = accordionItems.value.map((item) => {
         return totalWaterVolume.value > 0 ? Math.round((Number(item.water) / totalWaterVolume.value) * 100) : 0
      })
...
// 💡 負責攔截您的 updateWaterPercentage 事件！並強制更新平坦陣列觸發重繪！
const handleUpdateWaterPercentage = (val: any) => {
  const newList = [...percentagesList.value]
  console.info('攔截您的 updateWaterPercentage 事件', val.index, val.percentage)
...
watch([legumes, proportion], ([newLegumes, newProportion]) => {
  ...
  // 💡 當粉水比變動，也要一併利用平坦陣列通知全畫面更新百分比！
  percentagesList.value = accordionItems.value.map(item => {
    return total > 0 ? Math.round((Number(item.water || 0) / total) * 100) : 0
  })
})
