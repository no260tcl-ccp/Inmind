<template>
  <div v-if="step === 1">
    <div style="height: 100vh; background-color: white">
      ...
      >
        <template #left>
          <van-icon name="arrow-left" size="22px" style="color: #004097; font-weight: bold;" />
        </template>
      </van-nav-bar>
...
onMounted(() => {
  ...
  if (queryData) {
    try {
      ...
      accordionItems.value = (config.accordionItems || []).map((item: any, idx: number) => {
        const w = Number(item.water || 0)
        return {
          ...item,
          id: item.id || Date.now() + idx,
          water: w,
          percentage: totalWaterVolume.value > 0 ? Math.round((w / totalWaterVolume.value) * 100) : 0
        }
      })
...
const handleUpdateWater = (val: any) => {
  ...
  nowWater.value = newItems.reduce((sum, item) => sum + Number(item.water || 0), 0)
}

// 💡 2. 當點擊「自動分配」按鈕時 (依據您的 PDF 指示)
const autoWater = () => {
...
watch([legumes, proportion], ([newLegumes, newProportion]) => {
  ...
  accordionItems.value = accordionItems.value.map(item => ({
    ...item,
    percentage: total > 0 ? Math.round((Number(item.water || 0) / total) * 100) : 0
  }))
})
...
const handleUpdateWaterInjectType = (val: any) => {
  const newItems = [...accordionItems.value]
  newItems[val.index] = { ...newItems[val.index], type: val.type }
  accordionItems.value = newItems
}

const handleUpdateTemperature = (val: any) => {
  const newItems = [...accordionItems.value]
  newItems[val.index] = { ...newItems[val.index], temperature: val.newValue }
  accordionItems.value = newItems
}

const handleUpdateVelocity = (val: any) => {
  const newItems = [...accordionItems.value]
  newItems[val.index] = { ...newItems[val.index], velocity: val.newValue }
  accordionItems.value = newItems
}

const handleUpdateTime = (val: any) => {
  const newItems = [...accordionItems.value]
  newItems[val.index] = { ...newItems[val.index], time: val.newValue }
  accordionItems.value = newItems
}
...
const saveFormula = () => {
  ...
  accordionItems.value = accordionItems.value
    ...
    .map((item, index) => ({
      ...
      percentage: totalWaterVolume.value > 0 ? Math.round((Number(item.water) / totalWaterVolume.value) * 100) : 0
    }))
