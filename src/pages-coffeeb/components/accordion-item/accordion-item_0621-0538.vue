          ...
          <div class="left-section">
            ...
            <div class="line">
              <span class="percentage">
                {{ updateTrigger !== -1 ? (item.percentage || 0) : 0 }}<span class="font-size-6">%</span>
              </span>
            </div>
          </div>
...
const updateTrigger = ref(0) // 💡 建立隱藏觸發器，專治 Vue 3 緩存死角
...
// 💡 只要 items 有任何深層變動，就偷偷將 updateTrigger + 1，強迫 HTML 上的文字重新計算
watch(
  () => props.items,
  () => {
    totalWater.value = calculateTotalWater()
