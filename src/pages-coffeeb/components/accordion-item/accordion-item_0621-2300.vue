          ...
          <div class="left-section">
            ...
            <div class="line">
              <span class="percentage">
                {{ item.percentage || 0 }}<span class="font-size-6">%</span>
              </span>
            </div>
          </div>
...
// 💡 加上您提議的 updateWaterInjectPercentage 新事件！
...
const handleSliderChangeWater = (value: any, index: number) => {
  ...
  // 💡 2. 實作您的完美思路：子組件自己算好百分比，透過專屬事件發射出去！
  const pct = props.totalWaterVolume > 0 ? Math.round((w / props.totalWaterVolume) * 100) : 0
  emit('updateWaterInjectPercentage', { index, percentage: pct })
  
  // 3. 更新總水量
  emit('updateTotalWater', calculateTotalWater())
}
