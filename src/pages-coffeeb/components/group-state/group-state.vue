    ...
    >
      <view v-if="showIcon" class="icon">
        ...
        <wd-img
          ...
          :src="`/static/images/extract/${item.icon}.png`"
        />
        <wd-img
          :width="16"
          :height="16"
...
// 将 buttonTexts 转换为响应式数据
const buttonTexts = ref([...props.buttonTexts])

// 判断是否所有按钮 state 都为 true
const allStatesActive = computed(() => {
  return buttonTexts.value.every((item) => item.state)
})

// 计算容器样式
const containerStyles = computed(() => {
  return {
    '--button-count': buttonTexts.value.length,
  }
})

// 获取最后一个激活按钮的索引
const getLastActiveIndex = computed(() => {
  for (let i = buttonTexts.value.length - 1; i >= 0; i--) {
    if (buttonTexts.value[i].state) {
      return i
    }
  }
  return -1
...
const getButtonStyles = (index) => {
  ...
  const isActive = buttonTexts.value[index].state
