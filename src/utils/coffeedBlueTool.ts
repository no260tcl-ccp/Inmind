...
// 【修改點】：刪除了最外層的 store 初始化，避免 Pinia 尚未載入就執行的錯誤

export const useConnectionStatus = () => {
  // 【修改點】：移到函式內部呼叫
  const connectionStatus = computed(() => useBluetoothStore().connectionStatus)
...
export class CoffeeMachineProtocol {
  ...
  public async sendCommand(commandCode: number, data: number[]): Promise<string> {
    // 【修改點】：移到函式內部呼叫
    const connectionStatus = useBluetoothStore().connectionStatus
    ...
    // 【修改點】：移到函式內部呼叫
    await useBluetoothStore().sendData(new Uint8Array(message).buffer)
  ...
  public async listenForResponses() {
    // 【修改點】：移到函式內部呼叫
    useBluetoothStore().onDataReceived(this.handleDataReceived)
  }

  public async stopListeningForResponses() {
    // 【修改點】：移到函式內部呼叫
    useBluetoothStore().offDataReceived()
  }
  // 监听 和 停止监听方法  ---
  // 发送封装 --------------------- END
  ...
  public handleBrewMode(responseData: string): void {
    ...
    // 【修改點】：移到函式內部呼叫
    useMachineDStatusStore().updateBrewInfo(responseData)
    console.log('冲煮解析数据:', useMachineDStatusStore().brewData)
  }
  ...
  public handleGrindMode(responseData: string): void {
    ...
    // 【修改點】：移到函式內部呼叫
    useMachineDStatusStore().updateGrindInfo(responseData)
    console.log('研磨解析数据:', useMachineDStatusStore().grindData)
  }

  public handleWaterMode(responseData: string): void {
    console.log('注水工作模式:', responseData)
    // 【修改點】：移到函式內部呼叫
    useMachineDStatusStore().updateInjectionInfo(responseData)
    console.log('注水解析数据:', useMachineDStatusStore().injectionData)
  }
  ...
  public handleRecipeData(responseData: string): void {
    if (responseData == 'dd') {
      ...
      // 上行数据解析
      // 【修改點】：移到函式內部呼叫
      useMachineDStatusStore().updateFormulaInfo(responseData)
    }
  }
  ...
  public handleMachineStatus(responseData: string): void {
    ...
    // 调用 store 的方法更新机器状态
    // 【修改點】：移到函式內部呼叫
    useMachineDStatusStore().updateMachineStatus(responseData)
  }
  ...
  public handleAlarmInfo(responseData: string): void {
    // 调用 store 的方法更新警报信息
    // 【修改點】：移到函式內部呼叫
    useMachineDStatusStore().updateAlarmInfo(responseData)
  }
  ...
  public handleLackWater(responseData: string): void {
    // 调用 store 的方法更新缺水信息
    // 【修改點】：移到函式內部呼叫
    useMachineDStatusStore().updateLackWater(responseData)
  }
  ...
  public handleQueryVersion(responseData: string): void {
    if (responseData === 'dd') {
      ...
      // 【修改點】：移到函式內部呼叫
      useDevicVersionStore().setVersion(this.hexToString(responseData))
    }
  }
  ...
  public handleUpgradeStatus(responseData: string): void {
    ...
    // 【修改點】：移到函式內部呼叫
    useMachineDStatusStore().firmwareReady(responseData)
  }

  // 升级状态
  public handleUpgradeDataResponse(responseData: number): void {
    // 【修改點】：移到函式內部呼叫
    useMachineDStatusStore().upgradeStatusAction(responseData)
  }
